...
@section('content')
    <x-page-header title="Nouvelle demande d’imagerie"
    ...
    <form action="{{ route('imaging.store', $patient) }}" method="POST" novalidate>
        @csrf
        @if ($consultationId)
        ...
        <fieldset class="k-fieldset mb-4">
            <legend class="k-fieldset-legend">
                <x-icon name="scan" class="h-4.5 w-4.5 text-clinic-600"/> Examen demandé
            </legend>
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <select id="modality" name="modality" required class="k-select">
                        @foreach (\App\Models\ImagingOrder::MODALITIES as $value => $label)
                            <option value="{{ $value }}" @selected(old('modality') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                    <x-field-error name="modality"/>
                </div>
                <div>
