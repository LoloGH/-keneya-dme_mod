...
<section class="k-card">
    <div class="k-card-header">
        ...
        @can('hospitalizations.create')
            <a href="{{ route('hospitalizations.create', $patient) }}" class="k-btn-primary k-btn-sm">
                <x-icon name="plus" class="h-3.5 w-3.5"/> Nouvelle admission
            </a>
        @endcan
    </div>
                ...
                <li class="flex flex-wrap items-start justify-between gap-3 p-4">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <a href="{{ route('hospitalizations.show', $stay) }}"
                               class="font-mono text-sm font-medium text-clinic-700 hover:underline">
                                {{ $stay->hospitalization_number }}
                            </a>
                            <x-status-badge :status="$stay->status" :label="$stay->statusLabel()"/>
                        </div>
