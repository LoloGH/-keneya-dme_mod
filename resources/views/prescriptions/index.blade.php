...
@section('content')
    <x-page-header title="Ordonnances" subtitle="{{ $prescriptions->total() }} ordonnance(s)."/>
    ...
    <form method="GET" class="k-card mb-4 flex flex-wrap gap-3 p-4">
        ...
        <div>
            ...
            <select id="status" name="status" class="k-select">
                ...
                @foreach (\App\Models\Prescription::STATUSES as $value => $label)
                    ...
                    <tbody>
                        @foreach ($prescriptions as $prescription)
                            <tr>
                                <td class="font-mono text-xs">
                                    ...
                                    @if ($prescription->hasAllergyWarnings())
                                        <span class="k-badge-danger ml-1">
                                            <x-icon name="alert" class="h-3 w-3"/> Allergie
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td><x-status-badge :status="$prescription->status" :label="$prescription->statusLabel()"/></td>
                                <td class="text-right">
                                    <a href="{{ route('prescriptions.show', $prescription) }}" class="k-btn-ghost k-btn-sm">
