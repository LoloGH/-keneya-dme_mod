...
class DocumentStorage
{
    ...
    {
        return Storage::disk((string) config('keneya.documents.disk', 'local'));
    }
    ...
    public function storeUploaded(Patient $patient, UploadedFile $file, array $attributes = []): MedicalDocument
    {
        ...
        return MedicalDocument::create(array_merge([
            ...
            'disk' => (string) config('keneya.documents.disk', 'local'),
            'storage_path' => $path,
    ...
    private function buildPath(Patient $patient, ?string $extension): string
    {
        ...
        return sprintf(
            '%s/%d/%s.%s',
            trim((string) config('keneya.documents.directory', 'medical-documents'), '/'),
            $patient->getKey(),
            Str::uuid()->toString(),
