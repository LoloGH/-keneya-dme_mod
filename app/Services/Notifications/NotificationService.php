 ...
 *
 * C'est la seule couche qui connaît à la fois le domaine médical et le
 * service SMS. Les modèles et contrôleurs n'appellent jamais SmsService
 * directement : ils décrivent un événement métier, ce service décide des
 * destinataires internes (notification) et externes (SMS patient).
 *
 * Conséquence pour la phase 2 : seule cette classe sera à réécrire pour
 * brancher les notifications de Keneya Workflow ; SmsService, lui, est
...
class NotificationService
{
    ...
    public function appointmentReminder(Appointment $appointment): void
    {
        ...
        $message = $this->sendPatientSms($patient, 'appointment_reminder', [
            'patient_name' => $patient->fullName(),
            'time' => $appointment->scheduled_for->format('H\hi'),
    ...
    public function labResultAvailable(LabOrder $order): void
    {
        ...
        if ($order->doctor !== null) {
            $this->store(
                ...
                patient: $patient,
                actionUrl: route('laboratory.show', $order),
                level: 'info',
            );
        }
    ...
    public function prescriptionValidated(Prescription $prescription): void
    {
        ...
        $this->notifyPermission(
            ...
            patient: $patient,
            actionUrl: route('prescriptions.show', $prescription),
        );
    ...
    private function sendPatientSms(
        ...
        array $variables,
        ?object $context = null,
        ?\Illuminate\Support\Carbon $scheduledFor = null,
        ...
        return $this->sms->sendTemplate(
            ...
            recipient: (string) $patient->phone,
            variables: $variables,
            context: [
                'patient_id' => $patient->getKey(),
                'context_type' => $context !== null ? $context::class : null,
