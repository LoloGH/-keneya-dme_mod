...
class RefreshSmsStatuses extends Command
{
    ...
    {
        if (! config('sms.status_tracking.enabled')) {
        ...
        $limit = (int) ($this->option('limit') ?: config('sms.status_tracking.batch_size', 100));
        $maxAge = (int) config('sms.status_tracking.max_age_hours', 48);
